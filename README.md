...
- **입력 검증과 오류 안내**: 잘못된 정원, 반 수, 제약조건은 실행 전에 오류로 안내합니다. 제약조건 충돌이나 시간 초과로 배정안을 찾지 못하면 결과 파일을 생성하지 않습니다.
- **2단계 최적화 결과 보존**: `--use-lexicographic` 사용 시 2단계에서 시간 초과로 해를 찾지 못해도 1단계 배정안을 보존하며, 최적성이 증명되지 않은 결과는 최적해로 표시하지 않습니다.
...
- **다운로드 후 자동 삭제**: 각 파일을 다운로드하면 해당 파일이 즉시 삭제됩니다
  ...
  - 성공·실패 여부와 관계없이 작업 종료 **1시간 후 전체 작업 폴더와 메모리에 저장된 학생 정보를 삭제**합니다
- 통계 정보만 포함된 `report.txt`는 `data/reports/` 디렉토리에 영구 보관됩니다
- 작업 완료 후 1시간 이내에 필요한 파일을 모두 다운로드하세요
- 자동 정리는 웹 서버가 실행 중일 때 동작합니다. 서버를 강제 종료한 경우 남은 작업 폴더는 별도로 정리해야 합니다.
...
과목별 제한, 지정 반 수, 입력 검증, 해를 찾지 못한 경우, 2단계 최적화 결과 보존, 작업 정리 및 웹 업로드부터 결과 다운로드까지의 흐름을 검증합니다.
